...
"""def genBarChart(x, visemes, visemeState, filename):
...
filename = "dreamgenie_part1.lipsync"
...
print(len(lipsyncData), len(visemes))
...
for x in range(0, len(lipsyncData)):
    if lipsyncData[x] == 0:
        pass
    else:
        visemeEdit = -1
        #print(lipsyncData[x][0]*2)
        for y in range(0, lipsyncData[x][0]*2):
            if y%2 == 0:
                visemeEdit = lipsyncData[x][1][y]
            else:
                #print(visemeFrame[visemeEdit], visemeEdit)
                visemeFrame[visemeEdit] = lipsyncData[x][1][y]
